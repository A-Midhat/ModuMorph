...
TASKS=("DoorScalableHandle") # "LiftScalableCube") # change base case
...
echo "Morphs: ${MORPHS[*]}"
echo "Tasks: ${TASKS[*]}"
echo "Attributes: Scale, Density, Friction, Damping"
echo "Samples per attribute: ${NORMAL_SAMPLES_PER_ATTR} normal + ${EXTREME_SAMPLES_PER_ATTR} extreme"
echo "Results will be saved to: $AGGREGATED_RESULTS"
echo "======================================================"
...
damping_samples = np.concatenate([damping_normal, damping_extreme])
...
with open('$RESULTS_DIR/damping_samples.txt', 'w') as f:
    for i, val in enumerate(damping_samples):
        f.write(f"{i},{val:.4f}\n")
...
print(f"Damping: {damping_samples}")
EOF
...
while IFS=, read -r idx val; do
    DAMPING_SAMPLES[$idx]=$val
done < "$RESULTS_DIR/damping_samples.txt"

# echo "✅ Loaded $(( ${#SCALE_SAMPLES[@]} )) scale samples, $(( ${#DENSITY_SAMPLES[@]} )) density samples, $(( ${#FRICTION_SAMPLES[@]} )) friction samples"
echo "✅ Loaded $(( ${#SCALE_SAMPLES[@]} )) scale samples, $(( ${#DENSITY_SAMPLES[@]} )) density samples, $(( ${#FRICTION_SAMPLES[@]} )) friction samples, $(( ${#DAMPING_SAMPLES[@]} )) damping samples"
...
run_single_attr_job() {
    ...
    if [[ "$attr_type" == "scale" ]]; then
        ...
        # For DoorScalableHandle the Python script accepts a single float for friction.
        # For other tasks pass a triplet (as before).
        if [[ "$task" == "DoorScalableHandle" ]]; then
            CMD="$CMD --friction $sample_value"
        else
            CMD="$CMD --friction $sample_value $sample_value $sample_value"
        fi
    elif [[ "$attr_type" == "damping" ]]; then
        # Damping is only meaningful for tasks like DoorScalableHandle, but we allow testing for any task.
        CMD="$CMD --damping $sample_value"
    fi
...
for attr_type in "scale" "density" "friction" "damping"; do
