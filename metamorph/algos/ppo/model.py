...
class TransformerModel(nn.Module):
    ...
    def forward(self, obs_proprio, obs_mask, obs_env, obs_context,
        ...
        # ------ Debugging: Check for NaN values in obs_proprio_t and obs_context_t ----------------------------
        print("DEBUG: obs_proprio_t shape", obs_proprio_t.shape, "has nan:", torch.isnan(obs_proprio_t).any())
        print("DEBUG: obs_context_t shape", obs_context_t.shape, "has nan:", torch.isnan(obs_context_t).any())
        # ------------------------------------------------------------------------------------------------------
        ...
        if self.use_fixed_attention or self.use_hypernet or self.context_decoder_fusion:
            ...
            # ----- Debugging --------
            print("DEBUG: context_embedding_shared shape", context_embedding_shared.shape, "has nan:", torch.isnan(context_embedding_shared).any())
            # -------------------------
            
            if self.use_fixed_attention:
                ctx_embed_fa=self.context_embed_attention(obs_context_t)
                context_embedding_fa=self.context_encoder_attention(ctx_embed_fa) # TODO: check this line
                
                # ----- Debugging --------
                print("DEBUG: context_embedding_fa shape", context_embedding_fa.shape, "has nan:", torch.isnan(context_embedding_fa).any())
                # ------------------------


            # HField Fusion...; 
            # context_embedding_fa=self.context_encoder_attention(ctx_embed_fa) # not used for robosuite
            if self.use_hypernet: 
                ctx_embed_hn=self.context_embed_HN(obs_context_t)
                context_embedding_hn=self.context_encoder_HN(ctx_embed_hn)
                
                # ----- Debugging --------
                print("DEBUG: context_embedding_hn shape", context_embedding_hn.shape, "has nan:", torch.isnan(context_embedding_hn).any())
                # ------------------------
        ...
        if self.hn_embed:
            ...
            # ----- Debugging --------
            print("DEBUG: HN embed_w has nan:", torch.isnan(embed_w).any()); print("DEBUG: HN embed_b has nan:", torch.isnan(embed_b).any())
            # ------------------------
            ...
            # ----- Debugging --------
            print("DEBUG: proprio_embed shape", proprio_embed.shape, "has nan:", torch.isnan(proprio_embed).any())
            # ------------------------
        transformer_input = proprio_embed
        if self.model_args.get('EMBEDDING_SCALE', True): 
            transformer_input *= math.sqrt(self.d_model)
        if self.pos_embedding is not None: 
            transformer_input = self.pos_embedding(transformer_input)

            # ----- Debugging --------
            print("DEBUG: transformer_input after PE shape", transformer_input.shape, "has nan:", torch.isnan(transformer_input).any())
            # ------------------------

        # SWAT PE ...
        trav = None # TODO: add traversals later to the robosuite wrappers
        ...
        if self.model_args.get('EMBEDDING_DROPOUT',True):
            ...
            # ----- Debugging --------
            print("DEBUG: transformer_input after dropout shape", transformer_input.shape, "has nan:", torch.isnan(transformer_input).any())
            # ------------------------
        ...
        # # ----- Debugging --------
        print("DEBUG: obs_encoded shape", obs_encoded.shape, "has nan:", torch.isnan(obs_encoded).any())
        # ------------------------
        ...
        if self.context_decoder_fusion and context_embedding_shared is not None:
            ...
            # ----- Debugging --------
            print("DEBUG: decoder_input after context fusion shape", decoder_input.shape, "has nan:", torch.isnan(decoder_input).any())
            # ------------------------
        ...
        if self.ext_feat_fusion == "late" and self.extroceptive_encoder is not None:
            ...
            if extro_list:
                ...
                # ----- Debugging --------
                print("DEBUG: decoder_input after extro fusion shape", decoder_input.shape, "has nan:", torch.isnan(decoder_input).any())
                # ------------------------
        ...
        if self.hn_decoder: # Check hn_decoder flag directly
            ...
            for i in range(layer_num):
                ...
                # ----- Debugging --------
                print(f"DEBUG: HN Decoder Layer {i} W has nan:", torch.isnan(layer_w).any())
                print(f"DEBUG: HN Decoder Layer {i} B has nan:", torch.isnan(layer_b).any())
                print(f"DEBUG: HN Decoder Layer {i} Input has nan:", torch.isnan(output).any())
                # ------------------------
                
                output = torch.einsum('sbi,sbio->sbo', output, layer_w) + layer_b

                # ----- Debugging --------
                print(f"DEBUG: HN Decoder Layer {i} Output has nan:", torch.isnan(output).any())
                # ------------------------

                if i!=(layer_num-1):
                    output=F.relu(output)
                    # ----- Debugging --------
                    print("DEBUG: Decoder Output shape", output.shape, "has nan:", torch.isnan(output).any())
                    # -----------------------
        ...
        # ----- Debugging --------
        print("DEBUG: Final Output shape", output.shape, "has nan:", torch.isnan(output).any())
        # ------------------------
...
class TransformerModel(nn.Module):
    def __init__(self, obs_space, decoder_out_dim):
        ...
        if self.use_fixed_attention:
                self.context_embed_attention=nn.Linear(
                    self.context_obs_size,
                    self.model_args.CONTEXT_EMBED_SIZE
                    )
                # context_encoder_layer=TransformerEncoderLayerResidual(
                #     self.model_args.CONTEXT_EMBED_SIZE,
                ...
                #     self.model_args.CONTEXT_LAYER,norm=None
                #     )
                context_encoder_layer = TransformerEncoderLayerResidual(
                        self.model_args.CONTEXT_EMBED_SIZE,   # <‑‑ 16 or 64
                        self.model_args.NHEAD,
                        self.model_args.DIM_FEEDFORWARD,
                        self.model_args.DROPOUT
                )
                self.context_encoder_attention = TransformerEncoder(
                        context_encoder_layer,
                        self.model_args.CONTEXT_LAYER,
                        norm=None
                )
                # NEW – project encoder output up to d_model (=128)
                self.context_attention_output_proj = nn.Linear(
                        self.model_args.CONTEXT_EMBED_SIZE,
                        self.d_model
                )
                # skip for for robosuite
                if self.model_args.get('HFIELD_IN_FIX_ATTENTION') and "hfield" in obs_space.spaces:
                    ...
                    self.context_compress=nn.Sequential(
                        nn.Linear(
                            self.context_hfield_encoder.obs_feat_dim+self.model_args.CONTEXT_EMBED_SIZE,
                            self.model_args.CONTEXT_EMBED_SIZE),
                            nn.ReLU()
                            )
    ...
    def forward(self, obs_proprio, obs_mask, obs_env, obs_context,
        ...
        if self.use_fixed_attention or self.use_hypernet or self.context_decoder_fusion:
            ...
            # if self.use_fixed_attention:
            #     ctx_embed_fa=self.context_embed_attention(obs_context_t) # [22, 4, 16]
                
            #     context_embedding_fa=self.context_encoder_attention(ctx_embed_fa) # [22, 4, 18]
            #     # ----- Debugging --------
            #     print("DEBUG: context_embedding_fa shape", context_embedding_fa.shape, "has nan:", torch.isnan(context_embedding_fa).any())
            #     # ------------------------
            src_key_padding_mask = obs_mask.bool() if obs_mask is not None else None
            attn_mask = (
                morphology_info['SWAT_RE']
                if self.use_swat_re and morphology_info and 'SWAT_RE' in morphology_info
                else None
            )
            if self.use_fixed_attention:
                ctx_embed_fa=self.context_embed_attention(obs_context_t) # [22, 4, 16]
                ...
                # context_embedding_fa=self.context_encoder_attention(ctx_embed_fa) # [22, 4, 18]
                context_embedding_fa = self.context_encoder_attention(
                    ctx_embed_fa,
                    src_key_padding_mask=src_key_padding_mask,
                    morphology_info=morphology_info
                )
                # project  -->  [seq, batch, 128]
                context_embedding_fa = self.context_attention_output_proj(
                    context_embedding_fa
                )
                # ----- Debugging --------
                print("DEBUG: context_embedding_fa shape", context_embedding_fa.shape, "has nan:", torch.isnan(context_embedding_fa).any())
                # ------------------------
        ...
        attn_mask = morphology_info['SWAT_RE'] if self.use_swat_re and morphology_info and 'SWAT_RE' in morphology_info else None
        # context_for_transformer_layers = context_embedding_fa if self.use_fixed_attention else None
        context_for_transformer_layers = (
            context_embedding_fa if self.use_fixed_attention else None
        )
        attention_maps = None
        if return_attention:
            obs_encoded, attention_maps = self.transformer_encoder.get_attention_maps(
...
if __name__ == '__main__':
    import gym # Need gym for spaces definition
    from gym import spaces
    from collections import OrderedDict
    from metamorph.config import cfg # Need access to cfg for the test setup

    print("\n--- Running model.py Tests ---")

    def create_dummy_obs_space(max_limbs, max_joints, limb_obs_size, context_obs_size, extro_keys_dims={}):
        """Creates a dummy Dict observation space matching wrapper output."""
        inf = 0.5
        inf = np.float32(np.inf)

        obs_spaces = OrderedDict()
        obs_spaces['proprioceptive'] = spaces.Box(-inf, inf, (max_limbs * limb_obs_size,), np.float32)
        obs_spaces['context'] = spaces.Box(-inf, inf, (max_limbs * context_obs_size,), np.float32)
        obs_spaces['edges'] = spaces.Box(-inf, inf, (2 * max_joints,), np.float32)
        obs_spaces['obs_padding_mask'] = spaces.Box(False, True, (max_limbs,), dtype=bool)
        obs_spaces['act_padding_mask'] = spaces.Box(False, True, (max_limbs,), dtype=bool)
        # Add optional keys based on extro_keys_dims
        for key, dim_tuple in extro_keys_dims.items():
            obs_spaces[key] = spaces.Box(-inf, inf, dim_tuple, np.float32)
        # Add optional SWAT keys if needed for specific tests
        # obs_spaces['traversals'] = spaces.Box(0, max_limbs, (max_limbs, 3), dtype=np.int64) # Example
        # obs_spaces['SWAT_RE'] = spaces.Box(-inf, inf, (max_limbs, max_limbs, 3), np.float32) # Example

        return spaces.Dict(obs_spaces)

    def create_fake_batch(batch_size, obs_space, device='cpu'):
        CLAMP_VAL = 1e3                      # keep values finite
        fake_obs_batch = {}

        for key, space in obs_space.spaces.items():
            data = np.stack([space.sample() for _ in range(batch_size)])

            if space.dtype == bool:          # ----- mask tensors -----
                # convert to bool first
                tensor = torch.from_numpy(data.astype(bool)).to(device)

                # ensure **at least one False** per row
                if key in ['obs_padding_mask', 'act_padding_mask']:
                    tensor[:] = True                       # start all‑True
                    tensor[:, 0] = False                  # un‑mask the first limb
                fake_obs_batch[key] = tensor
            else:                         # ----- floating‑point tensors -----
                tensor = torch.from_numpy(data).float().to(device)
                tensor = torch.clamp(tensor, -CLAMP_VAL, CLAMP_VAL)
                fake_obs_batch[key] = tensor

        return fake_obs_batch


    # --- Test Parameters ---
    TEST_BATCH_SIZE = 4
    # Use padding values from a typical Robosuite TR setup for max size
    TEST_MAX_LIMBS = 22
    TEST_MAX_JOINTS = 20
    TEST_LIMB_OBS_SIZE = 16 # Estimate based on your proprio features
    TEST_CONTEXT_OBS_SIZE = 18 # Estimate based on your context features
    TEST_ACTION_DIM_PER_NODE = 1 # For mu_net output
    TEST_EXTRO = {"object-state": (10,)} # Example: Lift object state

    # Create dummy spaces
    # Base space (as output by RobosuiteNodeCentricObservation)
    test_obs_space = create_dummy_obs_space(
        TEST_MAX_LIMBS, TEST_MAX_JOINTS, TEST_LIMB_OBS_SIZE, TEST_CONTEXT_OBS_SIZE, TEST_EXTRO
    )
    # Action space (as seen by ActorCritic - base env space)
    # For testing, we only need the shape. Use a dummy value.
    # For TR Panda OSC_POSE, it's 14. Let's use a larger value for generality.
    test_action_space = spaces.Box(-1, 1, (16,), np.float32) # Example base action dim

    # --- Configure Model Settings for Test ---
    cfg.defrost()
    cfg.ENV_NAME="Robosuite-v0" # Needs to be set for model init logic
    cfg.MODEL.MAX_LIMBS = TEST_MAX_LIMBS
    cfg.MODEL.MAX_JOINTS = TEST_MAX_JOINTS
    cfg.MODEL.LIMB_EMBED_SIZE = 128 # d_model
    # Transformer defaults (can be overridden per test)
    cfg.MODEL.TRANSFORMER.NHEAD = 2
    cfg.MODEL.TRANSFORMER.DIM_FEEDFORWARD = 1024
    cfg.MODEL.TRANSFORMER.DROPOUT = 0.0 # Disable dropout for testing consistency
    cfg.MODEL.TRANSFORMER.NLAYERS = 2 # Use fewer layers for faster testing
    cfg.MODEL.TRANSFORMER.POS_EMBEDDING = "learnt" # Or None or "abs"
    cfg.MODEL.TRANSFORMER.EMBEDDING_DROPOUT = False
    cfg.MODEL.TRANSFORMER.CONTEXT_DECODER_FUSION = True # Test default fusion
    cfg.MODEL.TRANSFORMER.EXT_MIX = "late" # Test late fusion
    cfg.MODEL.TRANSFORMER.PER_NODE_EMBED = False # Test shared weights
    cfg.MODEL.TRANSFORMER.PER_NODE_DECODER = False
    cfg.MODEL.TRANSFORMER.USE_SWAT_PE = False
    cfg.MODEL.TRANSFORMER.USE_SWAT_RE = False
    # Set context sizes needed if FA/HN used
    cfg.MODEL.TRANSFORMER.CONTEXT_EMBED_SIZE = 64
    cfg.MODEL.TRANSFORMER.CONTEXT_LAYER = 1
    cfg.MODEL.TRANSFORMER.LINEAR_CONTEXT_LAYER = 1
    cfg.MODEL.TRANSFORMER.HN_CONTEXT_LAYER_NUM = 1
    # Define extroceptive encoder for the test extro keys
    cfg.MODEL.EXTROCEPTIVE_ENCODER.HIDDEN_DIMS = [32]
    # Action std (fixed)
    cfg.MODEL.ACTION_STD_FIXED = True
    cfg.MODEL.ACTION_STD = 0.6

    # --- Test Scenarios ---
    test_configs = {
        "Baseline": {"FIX_ATTENTION": False, "HYPERNET": False},
        "FA_Only": {"FIX_ATTENTION": True, "HYPERNET": False},
        "HN_Embed_Only": {"FIX_ATTENTION": False, "HYPERNET": True, "HN_EMBED": True, "HN_DECODER": False},
        "HN_Decoder_Only": {"FIX_ATTENTION": False, "HYPERNET": True, "HN_EMBED": False, "HN_DECODER": True},
        "HN_Both": {"FIX_ATTENTION": False, "HYPERNET": True, "HN_EMBED": True, "HN_DECODER": True},
        "FA_and_HN_Both": {"FIX_ATTENTION": True, "HYPERNET": True, "HN_EMBED": True, "HN_DECODER": True},
    }

    for name, flags in test_configs.items():
        print(f"\n--- Testing Config: {name} ---")
        cfg.MODEL.TRANSFORMER.FIX_ATTENTION = flags["FIX_ATTENTION"]
        cfg.MODEL.TRANSFORMER.HYPERNET = flags["HYPERNET"]
        if flags["HYPERNET"]: # Only set HN sub-flags if HN is True
             cfg.MODEL.TRANSFORMER.HN_EMBED = flags.get("HN_EMBED", False)
             cfg.MODEL.TRANSFORMER.HN_DECODER = flags.get("HN_DECODER", False)
        else: # Ensure sub-flags are off if HN is False
             cfg.MODEL.TRANSFORMER.HN_EMBED = False
             cfg.MODEL.TRANSFORMER.HN_DECODER = False
        cfg.freeze() # Freeze config for this test run

        try:
            # Initialize model with current config
            actor_critic = ActorCritic(test_obs_space, test_action_space)
            actor_critic.eval() # Set to eval mode
            print(f"  Model Initialized: {name}")

            # Create fake batch
            fake_batch = create_fake_batch(TEST_BATCH_SIZE, test_obs_space)
            fake_actions = torch.randn(TEST_BATCH_SIZE, actor_critic.num_actions_policy_output) # Fake actions matching policy output dim

            # Test forward pass with actions (for logp/entropy)
            val, pi, logp, entropy, _, _ = actor_critic(fake_batch, act=fake_actions)
            print(f"  Forward pass (with actions): PASSED")
            print(f"    Value shape: {val.shape}")
            print(f"    Logp shape: {logp.shape}")
            print(f"    Pi mean shape: {pi.mean.shape}")
            assert val.shape == (TEST_BATCH_SIZE, 1)
            assert logp.shape == (TEST_BATCH_SIZE, 1)
            assert pi.mean.shape == (TEST_BATCH_SIZE, actor_critic.num_actions_policy_output)

            # Test forward pass without actions (for act() method)
            val, pi, _, _, _, _ = actor_critic(fake_batch, act=None)
            print(f"  Forward pass (no actions): PASSED")
            print(f"    Value shape: {val.shape}")
            print(f"    Pi mean shape: {pi.mean.shape}")
            assert val.shape == (TEST_BATCH_SIZE, 1)
            assert pi.mean.shape == (TEST_BATCH_SIZE, actor_critic.num_actions_policy_output)
            ...
            val_a, act_a, logp_a, _, _ = agent.act(fake_batch)
            val_g = agent.get_value(fake_batch)
            print(f"  Agent methods: PASSED")
            print(f"    act() action shape: {act_a.shape}")
            assert act_a.shape == (TEST_BATCH_SIZE, actor_critic.num_actions_policy_output)
            assert val_a.shape == (TEST_BATCH_SIZE, 1)
            assert logp_a.shape == (TEST_BATCH_SIZE, 1)
            assert val_g.shape == (TEST_BATCH_SIZE, 1)

        except Exception as e:
            print(f"  !!! TEST FAILED for {name}: {e}")
            # Optionally re-raise if you want the script to stop on first failure
            # raise e
        finally:
            cfg.defrost() # Defrost config for the next loop iteration

    print("\n--- model.py Tests Complete ---")